...
# we want to recieve the filename with socket connection
...
process = []
number_of_process = number_of_row
list_of_inputs = str.split("\n")
list_of_inputs.pop()

for input in list_of_inputs:
    str_list = input.split(" ") 
    process.append(Process(int(str_list[0]),int (str_list[1]), int(str_list[2]), int(str_list[3])))
...
cpus_file = open('cpus.obj', 'w') 
pickle.dump(cpus, cpus_file)
